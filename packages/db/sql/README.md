...
- `0022_sp_importar_ejecutar.sql`: `PROCEDURE` orquestador del motor de importación — valida, y opcionalmente impacta/mueve a histórico/limpia, resolviendo dinámicamente el `SP_NOMBRE` configurado en `IMPORTADORES` (ver ADR 0021, `domain/importadores.md`).
- `0023_sp_import_legajos_clientes.sql`: `PROCEDURE` del primer importador real — upsert de `LEGAJOS` (por `NUMERO`) y `CLIENTES` (por tipo+nro de documento, actualización parcial) desde `IMPORT_LEGAJOS_CLIENTES_STG`.
