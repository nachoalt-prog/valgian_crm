-- Agrega p_placeholders (PLACEHOLDERS ya resueltos, en vez de resolverlos
-- desde PLACEHOLDERS_DATOS_RAIZ) y p_forzar_inmediato (dispara igual aunque
-- ACCIONES_EXTERNAS.INMEDIATO sea false) a sp_mensajeria_encolar — ver
-- domain/acciones-externas.md, sección "Probar una plantilla". CREATE OR
-- REPLACE PROCEDURE no reemplaza si cambia la firma de parámetros (crea un
-- overload nuevo y deja el viejo vivo) — hace falta el DROP explícito.
DROP PROCEDURE IF EXISTS sp_mensajeria_encolar(uuid, uuid, uuid, uuid, uuid, uuid[], jsonb, text);
...
BEGIN
  ...
  VALUES
    (p_id_mensajeria_cola, p_id_accion_externa, p_id_plantilla, p_id_entidad, p_id_registro, v_asunto, p_destino, p_datos, p_placeholders, now());
