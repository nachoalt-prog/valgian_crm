...
- `0002_fn_xcc_aplicar_retenciones.sql`: aplica las retenciones activas (`XCC_TIPOS_RETENCION`) sobre un interés bruto ya acreditado — invocada desde `sp_xcc_recalcular_cuenta` en sus dos puntos de acreditación (ver `docs/módulo XCC`, sección 4).
...
100% base de datos (`pg_cron`/`PROCESOS`, sin componente de aplicación) — ver `docs/módulo XCC`, sección 3, para el porqué (`pg_cron` no tiene un primitivo de "ejecutar esto una vez, ahora" por evento; ver también ADR 0015).
