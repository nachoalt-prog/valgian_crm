...
**Bug real encontrado después, reportado por el usuario**: `timezone`/`cron.timezone` nunca se habían configurado — Postgres arrancaba en UTC/GMT por default. No era solo un problema de display: `fn_cron_matches` usa `EXTRACT(HOUR FROM ...)`, que depende de la zona horaria de sesión, así que `limpieza_tokens_vencidos` (`0 3 * * *`, pensado como 3am Argentina) en realidad corría a medianoche Argentina. Corregido forzando `timezone`/`cron.timezone=America/Argentina/Buenos_Aires` en el `command:` de `docker-compose.yml` — ver `domain/procesos.md`, sección "Zona horaria".
