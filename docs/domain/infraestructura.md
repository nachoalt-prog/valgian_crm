...
## Paneles colapsables de Bandejas/Reportes

Mismo lenguaje visual que el colapso global de arriba (ancho `w-14` colapsado, ícono centrado, botón con `Tooltip` `side="right"`), pero disparado distinto: acá colapsa al ELEGIR una bandeja/reporte (no por click afuera) — la idea es liberarle espacio al listado de resultados una vez que ya se sabe qué se está mirando. `BandejasPanel`/`ReportesPanel` reciben `collapsed`/`onToggleCollapsed` desde `BandejasTool`/`ReportesTool` (`panelCollapsed`, seteado a `true` al arrancar `handleSelectBandeja`/`handleSelectReporte`); el botón para volver a expandir vive tanto en el riel colapsado como en el header ya expandido (mismo par de botones que ya tiene el sidebar global — se puede volver a colapsar sin tener que elegir otra bandeja/reporte).
