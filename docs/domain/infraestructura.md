...
Qué interfaz se usa en cada momento (`COLOR_PRIMARIO`/`COLOR_SECUNDARIO`/`IMAGEN_FONDO` — los tres siguen el mismo criterio):
...
- **Antes de loguearse, ya logueado alguna vez en este navegador**: se usan los valores guardados en la cookie `interfaz_colores` (`apps/web/src/lib/interfaz-colores.ts`, `InterfazColoresCookie = {colorPrimario, colorSecundario, imagenFondo}`) — la última interfaz REAL que vio ese navegador, no la `default` genérica. La cookie guarda la URL de `IMAGEN_FONDO`, nunca la imagen en sí. Se escribe en `apps/web/src/app/login/actions.ts` justo después de un login exitoso, con los datos de la `INTERFAZ` del perfil que acaba de entrar. `apps/web/src/app/layout.tsx` la lee para los colores (CSS vars, no hay sesión activa todavía); `apps/web/src/app/page.tsx` (la página de login en sí) la lee por separado para `imagenFondo`, con fallback a `getTemaPorDefecto()` si la cookie no la tiene (evaluación perezosa — no pega a la base si la cookie ya la trae).
...
`IMAGEN_FONDO` solo se usa en la pantalla de login (nunca en el shell post-login): si tiene un valor, se muestra como imagen de fondo detrás del formulario, al 30% de opacidad (70% transparente), sin reemplazar el `COLOR_PRIMARIO`/fondo normal — se ve el color de siempre con la imagen apenas insinuada encima. Si es nulo, el fondo se ve exactamente como sin esta funcionalidad. Importante: solo la `INTERFAZ` con `CODIGO='default'` (o la última interfaz real vista en ese navegador, vía la cookie) importa acá — cargar una imagen en cualquier otra fila desde el ABM de Interfaces no tiene ningún efecto visible hasta que un perfil con esa interfaz se loguee al menos una vez en ese navegador.
