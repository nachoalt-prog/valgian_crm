...
**Ojo con `ID_ENTIDAD`/`ID_REGISTRO` del `INSERT`** — tienen que apuntar a lo que el registro que disparó la acción **ES SOBRE**, no al propio registro que disparó la transición. Ejemplo real: una acción colgada de una transición de `TRAMITES` recibe `$1`=el `ID` del propio trámite — pero el documento generado tiene que quedar adjunto al legajo/cliente que el trámite gestiona, no al trámite en sí (no existe ninguna pantalla que muestre "adjuntos de un trámite"). El patrón correcto es resolver el destino real con un `JOIN`, no asumirlo:
