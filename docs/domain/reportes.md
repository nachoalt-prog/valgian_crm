...
- **`apps/web/src/lib/resultados-formato.tsx`**: `formatearValor`/`compararValores` compartidos entre `BandejaResultados` y `ReporteResultados` (mismo vocabulario de `tipo` de columna: `badge`/`fecha`/`fecha_hora`/`adjuntos`/`pasos`/default). Los tipos `adjuntos` y `pasos` son botones de solo-ícono que abren un diálogo de detalle read-only aparte — en ambos casos el valor de la columna es el `ID` de la fila dueña del detalle (`MENSAJERIA_COLA.ID` / `PROCESOS_EJECUCIONES.ID`), no un texto a mostrar.
