...
interface ReporteFiltroDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  vinculo: ReporteFiltroConNombre | null;
...
export function ReporteFiltroDialog({ open, onOpenChange, vinculo, filtrosDisponibles, onSave }: ReporteFiltroDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{vinculo ? "Editar Filtro del Reporte" : "Agregar Filtro al Reporte"}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-2">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="campo" className="text-xs uppercase tracking-wider text-muted-foreground">
              Campo (alias de la query del reporte)
            </Label>
            <Input id="campo" value={campo} onChange={(e) => setCampo(e.target.value)} placeholder="ej: estado, alta_fecha" required />
          </div>
