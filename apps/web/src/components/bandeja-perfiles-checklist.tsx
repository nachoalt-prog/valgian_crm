...
import { getPerfilesConEstadoAction, toggleBandejaPerfilAction } from "@/app/dashboard/bandejas-admin/actions";
...
interface BandejaPerfilesChecklistProps {
  bandejaId: string;
  canGestionar: boolean;
  onSinPermiso: () => void;
}

export function BandejaPerfilesChecklist({ bandejaId, canGestionar, onSinPermiso }: BandejaPerfilesChecklistProps) {
  const [perfiles, setPerfiles] = useState<PerfilConEstado[] | null>(null);
  const [pendingId, setPendingId] = useState<string | null>(null);

  useEffect(() => {
    let cancelado = false;
    getPerfilesConEstadoAction(bandejaId).then((res) => {
  ...
  async function toggle(perfilId: string, actual: boolean) {
    ...
    const result = await toggleBandejaPerfilAction(bandejaId, perfilId, nuevoEstado);
    setPendingId(null);
